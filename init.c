...
/*   Updated: 2025/06/24 13:49:46 by msuokas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_philos(t_data *data)
{
	...
	while (++i < data->nbr_of_philos)
	{
		if (data->nbr_of_philos % 2 == 0)
			...
			right_fork = &data->philo[(i - 1 + data->nbr_of_philos)
				% data->nbr_of_philos].left_fork;
...
int	init_base(t_data *data, char **argv, int argc)
{
	...
	if (!init_mutex(data, &data->stop_mutex))
		return (0);
	if (!init_mutex(data, &data->limit_mutex))
		return (0);
	...
	data->limit = data->nbr_of_philos - 1;
	if (argc == 6)
